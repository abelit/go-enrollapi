...
func (i *initMenuAuthority) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	// 9528
	if err = db.Model(&authorities[1]).Association("SysBaseMenus").Replace(menus[7:7]); err != nil {
		return next, err
	}
	if err = db.Model(&authorities[1]).Association("SysBaseMenus").Append(menus[11:11]); err != nil {
		return next, err
	}
	return next, nil
}
